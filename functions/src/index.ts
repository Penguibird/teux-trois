...
import { collection, initializeFirestore, getDocs, doc, writeBatch } from 'firebase/firestore'
  ...
  .onRun(async (ctx) => {
    ...
    allUsers.forEach((_d) => {
      promises.push((async (d) => {
        const targetCol = collection(doc(collection(d.ref, 'todos'), targetID), "items")
        const allTodosSourceCol = collection(doc(collection(d.ref, 'todos'), sourceID), "items")
